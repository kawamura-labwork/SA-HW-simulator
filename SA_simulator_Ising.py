...
class Annealing():
	...
	## CONSTRUCTOR ##
	def __init__(self, N, J, h, C, p, d, v, la, mu):
		self.N = N
		self.J = J
		self.h = h
		self.C = C
	...
	def calcH(self):
		energy = -1 * np.dot(np.dot(self.state, self.J), self.state) // 2 - np.dot(self.h, self.state) + self.C
		return energy
	...
	def run(self):
		...
		for i in tqdm(range(self.o_loop)):
			...
			if self.debug:
				energy = -1 * np.dot(self.local_field + self.h, self.state) // 2 + self.C
...
def main():
	...
	anneal = Annealing(N, J, h, C, params, args.debug, args.var, la, mu)
	anneal.run()
